  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="text-center">
        <div className="bg-gradient-to-br from-emerald-100 to-teal-100 p-4 rounded-full w-16 h-16 sm:w-24 sm:h-24 mx-auto mb-4 shadow-lg">
          <Gift className="w-8 h-8 sm:w-14 sm:h-14 text-emerald-600" />
        </div>
        <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2 sm:mb-3">Reward Trashbag</h2>
        <p className="text-gray-600 text-sm sm:text-lg">Kumpulkan botol dan dapatkan trashbag sebagai reward!</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div className="bg-gradient-to-br from-emerald-500 via-teal-500 to-green-600 rounded-2xl p-4 sm:p-6 text-white shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-emerald-100 text-xs sm:text-sm font-medium">Total Trashbag</p>
              <p className="text-3xl sm:text-4xl font-bold">{totalTrashbags}</p>
            </div>
            <Gift className="w-8 h-8 sm:w-12 sm:h-12 text-emerald-200" />
          </div>
        ...
        {/* Total Bottles */}
        <div className="bg-gradient-to-br from-teal-500 to-cyan-600 rounded-2xl p-4 sm:p-6 text-white shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-teal-100 text-xs sm:text-sm font-medium">Total Botol</p>
              <p className="text-3xl sm:text-4xl font-bold">{bottleStats.totalBottles.toLocaleString('id-ID')}</p>
            </div>
            <Package className="w-8 h-8 sm:w-12 sm:h-12 text-teal-200" />
          </div>
          <div className="mt-3 sm:mt-4 flex items-center gap-2">
        ...
        {Object.keys(bottleStats.wasteBreakdown).length === 0 ? (
          ...
          <div className="space-y-3 sm:space-y-4">
            {Object.entries(bottleStats.wasteBreakdown).map(([wasteType, stats]) => (
              <div key={wasteType} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 sm:p-4 bg-gradient-to-r from-gray-50 to-white rounded-xl border border-gray-100 hover:shadow-md transition-all duration-200">
                <div className="flex items-center gap-3">
                  <span className="text-xl sm:text-2xl">{wasteTypeIcons[wasteType] || '🗑️'}</span>
                  <div>
      ...
      <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl p-4 sm:p-6 border border-emerald-200 shadow-lg">
        <div className="flex items-center gap-2 sm:gap-3 mb-3">
          <Award className="w-5 h-5 sm:w-6 sm:h-6 text-emerald-600" />
          <h3 className="text-base sm:text-lg font-semibold text-emerald-800">Tetap Semangat!</h3>
        </div>
