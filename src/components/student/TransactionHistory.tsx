  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="text-xs sm:text-sm text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full">
        ...
        <>
          {allHistory.length === 0 ? (
            <div className="text-center py-8 sm:py-12">
              <div className="bg-gradient-to-br from-emerald-100 to-teal-100 p-4 rounded-full w-16 h-16 sm:w-20 sm:h-20 mx-auto mb-4">
                <Package className="w-8 h-8 sm:w-12 sm:h-12 text-emerald-500" />
              </div>
              <p className="text-gray-600 text-base sm:text-lg">Belum ada riwayat transaksi</p>
              <p className="text-gray-500 text-xs sm:text-sm mt-2">Mulai kumpulkan botol untuk mendapatkan reward!</p>
            </div>
          ) : (
            <div className="space-y-3 sm:space-y-4">
              {allHistory.map((item) => {
                ...
                return (
                  <div key={item.id} className="bg-gradient-to-br from-white to-gray-50 border border-gray-100 rounded-2xl p-4 sm:p-5 hover:shadow-lg transition-all duration-300">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div className="flex items-start gap-3">
                        <div className={`p-2 sm:p-3 rounded-xl ${
                          ...
                          {isDeposit ? (
                            ...
                            <ArrowDown className="w-4 h-4 sm:w-6 sm:h-6 text-blue-600" />
                          ) : (
                          ...
                          {hasReward && (
                            <div className="flex flex-wrap items-center gap-1 sm:gap-2 mt-1">
                              <div className="flex items-center gap-1 bg-purple-100 px-2 py-1 rounded-full">
                                <TrendingUp className="w-2 h-2 sm:w-3 sm:h-3 text-purple-600" />
                              ...
                              {item.reward.trashbags > 0 && (
                                <div className="flex items-center gap-1 bg-pink-100 px-2 py-1 rounded-full">
                                  <Gift className="w-2 h-2 sm:w-3 sm:h-3 text-pink-600" />
                      ...
                      <div className="text-right sm:text-right">
                        ...
                        {hasReward && item.reward.trashbags > 0 && (
                          <div className="flex items-center gap-1 justify-end mt-1">
                            <Gift className="w-3 h-3 sm:w-4 sm:h-4 text-pink-500" />
                        ...
                        {(isWithdrawal || isTrashbagWithdrawal) && StatusIcon && (
                          <div className="flex items-center gap-1 justify-end mt-1">
                            <StatusIcon className={`w-2 h-2 sm:w-3 sm:h-3 ${getStatusColor(item.status)}`} />
